...
Let's first **move** the file into tmp fir:
```shell
bandit12@bandit:~$ mkdir /tmp/nec
bandit12@bandit:~$ cp data.txt /tmp/nec
bandit12@bandit:~$ cd /tmp/nec
bandit12@bandit:/tmp/nec$ ls
data.txt
```

Then we use `xxd` command -- xxd is making a hexdump or do the reverse.
```shell
bandit12@bandit:/tmp/nec$ xxd -r data.txt > data.bin
bandit12@bandit:/tmp/nec$ ls
data.bin  data.txt
```

Now, you have to find out **what kind of file this binary data represents**. file is a command that can tell you the type of data contained in a file:
```shell 
bandit12@bandit:/tmp/nec$ file data.bin
data.bin: gzip compressed data, was "data2.bin", last modified: Sun Apr 23 18:04:23 2023, max compression, from Unix, original size modulo 2^32 581
```
This will likely tell you that the data has been **compressed** using a specific method, such as gzip, bzip2, tar etc. Depending on the output, you'll need to decompress the file using the corresponding tool. 

In our case, it's a `gzip` type
```shell
bandit12@bandit:/tmp/nec$ mv data.bin data.gz
bandit12@bandit:/tmp/nec$ gunzip data.gz
bandit12@bandit:/tmp/nec$ ls
data  data.txt
```
...
```shell
bandit12@bandit:/tmp/nec$ file data
data: bzip2 compressed data, block size = 900k
```



Remember, here are the common decompression commands for various types:
- For gzip files: `gunzip filename`
- For bzip2 files: `bunzip2 filename`
- For tar files: `tar xvf filename`
